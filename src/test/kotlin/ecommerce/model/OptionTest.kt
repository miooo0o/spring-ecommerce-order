...
class OptionTest {
    ...
    fun `test allowed special characters`(name: String) {
        assertDoesNotThrow { Option(name = name, quantity = 3) }
    }

    @ParameterizedTest
    @ValueSource(strings = ["abc?", "!abc", "a@bc"])
    fun `test not allowed special characters`(name: String) {
        assertThrows<IllegalArgumentException> { Option(name = name, quantity = 3) }
    }

    @Test
    fun `names can include up to 50 characters`() {
        assertDoesNotThrow {
            Option(name = "a".repeat(49), quantity = 3)
        }
    }

    @Test
    fun `names can not include more than 50 characters`() {
        assertThrows<IllegalArgumentException> { Option(name = "a".repeat(51), quantity = 3) }
    }
}
